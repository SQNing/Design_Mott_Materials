...
- Classical-method guidance: recommend `luttinger-tisza` for isotropic bilinear Heisenberg models, including multi-sublattice unit cells, when its assumptions hold. Keep XXZ-like LT use in the current narrow helper scope; otherwise recommend `variational`. Treat `annealing` as future scope, not a live implementation.
