...
def _preflight_payload_error(classical_payload):
    ...
    if model.get("classical_manifold") != "CP^(N-1)":
        return _error(
            "invalid-classical-payload",
            "Sunny pseudospin-orbital SUN classical payload expects a CP^(N-1) model",
...
def run_sunny_sun_classical(payload, julia_cmd=None, stream_progress=False):
    ...
    if isinstance(classical_payload.get("model"), dict):
        classical_payload = dict(classical_payload)
        classical_payload["model"] = adapt_model_for_sunny_pair_couplings(classical_payload["model"])
