...
class LinearSpinWaveDriverTests(unittest.TestCase):
    ...
    def test_linear_spin_wave_summary_can_use_effective_model_main_block(self):
        model = {
            "spin": 0.5,
            "effective_model": {"main": [{"type": "isotropic_exchange", "coefficient": 2.0}]},
            "q_grid": [0.0, 3.141592653589793],
        }
        summary = linear_spin_wave_summary(model)
        self.assertEqual(summary["dispersion"][1]["omega"], 4.0)
