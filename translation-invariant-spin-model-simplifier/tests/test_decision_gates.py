...
class DecisionGatesTests(unittest.TestCase):
    ...
    def test_classical_stage_recommends_luttinger_tisza_for_multi_sublattice_heisenberg(self):
        model = {"lattice": {"sublattices": 3}, "simplified_model": {"template": "heisenberg"}}
        decision = classical_stage_decision(model)
        self.assertEqual(decision["status"], "needs_input")
        self.assertEqual(decision["question"]["id"], "classical_method")
        self.assertEqual(decision["recommended"], "luttinger-tisza")

    def test_classical_stage_still_requires_user_choice_after_timeout(self):
        model = {"lattice": {"sublattices": 1}, "simplified_model": {"template": "heisenberg"}}
    ...
    def test_classical_stage_can_auto_select_when_user_explicitly_allows_auto_run(self):
        model = {"lattice": {"sublattices": 1}, "simplified_model": {"template": "heisenberg"}}
        decision = classical_stage_decision(model, allow_auto_select=True)
