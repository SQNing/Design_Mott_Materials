...
class NormalizeInputTests(unittest.TestCase):
    def test_operator_payload_is_normalized(self):
        ...
        self.assertEqual(normalized["lattice_description"]["kind"], "bravais")
    ...
    def test_structured_lattice_description_is_preserved(self):
        payload = {
            "representation": "operator",
            ...
            "user_required_symmetries": ["translation", "time_reversal"],
            "allowed_breaking": ["u1_spin"],
        }
        normalized = normalize_input(payload)
        self.assertEqual(normalized["lattice_description"]["kind"], "triangular")
        self.assertEqual(normalized["user_required_symmetries"], ["translation", "time_reversal"])
        self.assertEqual(normalized["allowed_breaking"], ["u1_spin"])

    def test_natural_language_lattice_description_is_preserved(self):
        payload = {
            "representation": "natural_language",
            "description": "Spin-1/2 antiferromagnet with easy-axis exchange",
            "lattice_description": "Honeycomb lattice with two magnetic sites per unit cell and first distance shell J1",
        }
        normalized = normalize_input(payload)
        self.assertEqual(
            normalized["lattice_description"],
            {"kind": "natural_language", "value": payload["lattice_description"]},
        )

    def test_interaction_metadata_is_passed_through(self):
        payload = {
            "representation": "natural_language",
            "description": "Spin-1/2 model",
            "interaction": {"status": "needs_input", "question": "Clarify lattice type"},
        }
        normalized = normalize_input(payload)
        self.assertEqual(normalized["interaction"], payload["interaction"])
    ...
    def test_natural_language_input_preserves_summary(self):
        ...
        self.assertEqual(normalized["lattice_description"]["kind"], "natural_language")
